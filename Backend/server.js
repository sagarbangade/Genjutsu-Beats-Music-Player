...
const authorizeOwner = (resourceType) => async (req, res, next) => {
    ...
    try {
        if (resourceType === 'music') {
            ...
            return res.status(400).send({ message: 'Invalid resource type for authorization.' });
        }

        if (!resource) {
            return res.status(404).send({ message: `${resourceType.charAt(0).toUpperCase() + resourceType.slice(1)} not found.` });
        }

        if (resource.userId.toString() !== userId) {
            return res.status(403).send({ message: `Unauthorized to modify this ${resourceType}.` });
        }
        next();
    } catch (error) {
        console.error("Authorization error:", error);
        res.status(500).send({ message: 'Authorization failed.', error: error.message });
    }
};
...
const storage = multer.diskStorage({
    destination: (req, file, cb) => {
        ...
        if (file.fieldname === 'audioFile' || file.fieldname === 'newMusicFiles') { // 'newMusicFiles' added here
            destFolder += 'music';
...
const fileFilter = (req, file, cb) => {
    if (file.fieldname === 'albumArt') {
        ...
        if (allowedMimeTypes.includes(file.mimetype)) {
            ...
            cb(null, false);
            return cb(new Error('Only image files are allowed for album art (jpeg, png, gif, webp).'));
        }
    } else if (file.fieldname === 'audioFile' || file.fieldname === 'newMusicFiles') { // 'newMusicFiles' added here
        const allowedAudioTypes = ['audio/mpeg', 'audio/mp3', 'audio/wav', 'audio/ogg'];
        if (allowedAudioTypes.includes(file.mimetype) || path.extname(file.originalname).toLowerCase() === '.mp3' || path.extname(file.originalname).toLowerCase() === '.wav' || path.extname(file.originalname).toLowerCase() === '.ogg' ) {
            cb(null, true);
        } else {
            cb(null, false);
            return cb(new Error('Only audio files are allowed (mp3, wav, ogg, mpeg).'));
        }
    }
    else {
...
app.post('/api/auth/login', async (req, res) => {
    try {
        ...
        res.status(200).send({ message: 'Login successful.', token: token, userId: user._id, username: user.username });
    } catch (error) {
...
app.post('/api/music/upload', authenticateJWT, musicUpload.fields([{ name: 'audioFile', maxCount: 1 }, { name: 'albumArt', maxCount: 1 }]), async (req, res) => {
    try {
        ...
        const albumArtFile = req.files['albumArt'] ? req.files['albumArt'][0] : null;
        ...
        await newMusic.save();
        res.status(201).send({ message: 'Music uploaded successfully.', music: newMusic });

    } catch (error) {
        console.error("Upload music error:", error);
        if (error instanceof multer.MulterError) {
            return res.status(400).send({ message: 'File upload error.', error: error.message, field: error.field });
        } else {
            res.status(500).send({ message: 'Error uploading music.', error: error.message });
        }
    }
});
...
app.get('/api/music', authenticateJWT, async (req, res) => {
    try {
        ...
        let sortOptions = {};
        if (sortBy) {
            if (sortBy === 'uploadDate' || sortBy === '-uploadDate') sortOptions.uploadDate = sortBy === 'uploadDate' ? 1 : -1;
            if (sortBy === 'title' || sortBy === '-title') sortOptions.title = sortBy === 'title' ? 1 : -1;
        } else {
        ...
        res.status(200).send({ music: musicList, totalCount, currentPage: parseInt(page), limit: parseInt(limit) });
    } catch (error) {
        console.error("Get music list error:", error);
        res.status(500).send({ message: 'Error fetching music list.', error: error.message });
    }
});
...
app.get('/api/music/:musicId', authenticateJWT, async (req, res) => {
    try {
        ...
        res.status(500).send({ message: 'Error fetching music details.', error: error.message });
    }
});

// Delete Music Track (Protected, Owner-Only)
app.delete('/api/music/:musicId', authenticateJWT, authorizeOwner('music'), async (req, res) => {
    try {
        const musicId = req.params.musicId;
        ...
        res.status(500).send({ message: 'Error deleting music track.', error: error.message });
    }
});
...
app.post('/api/playlists', authenticateJWT, musicUpload.array('newMusicFiles', 5), async (req, res) => {
    try {
        const { name, description, coverImageUrl, existingSongIds, newMusicTitles, newMusicArtists, newMusicAlbums, newMusicGenres, newMusicYears } = req.body;
        ...
        if (existingSongIds) {
            ...
            try {
                ...
                if (!Array.isArray(parsedExistingSongIds)) {
                    return res.status(400).send({ message: 'existingSongIds must be an array of music IDs.' });
                }
            } catch (error) {
                return res.status(400).send({ message: 'Invalid format for existingSongIds. Must be a JSON array of music IDs.' });
            }

            if (parsedExistingSongIds.length > 0) {
                const validExistingSongIds = await Music.find({ _id: { $in: parsedExistingSongIds }, userId: req.user.userId }).distinct('_id');
        ...
        if (req.files && req.files.length > 0) {
            ...
            for (let i = 0; i < uploadedMusicFiles.length; i++) { // Loop with index to match metadata arrays
                const musicFile = uploadedMusicFiles[i];
                const audioFileUrl = `/uploads/music/${musicFile.filename}`;

                const title = newMusicTitles ? (Array.isArray(newMusicTitles) ? newMusicTitles[i] : newMusicTitles) : musicFile.originalname; // Get title from array or single value or filename
                const artist = newMusicArtists ? (Array.isArray(newMusicArtists) ? newMusicArtists[i] : newMusicArtists) : undefined;
                const album = newMusicAlbums ? (Array.isArray(newMusicAlbums) ? newMusicAlbums[i] : newMusicAlbums) : undefined;
                const genre = newMusicGenres ? (Array.isArray(newMusicGenres) ? newMusicGenres[i] : newMusicGenres) : undefined;
                const year = newMusicYears ? (Array.isArray(newMusicYears) ? parseInt(newMusicYears[i]) : parseInt(newMusicYears)) : undefined;
        ...
        const uniqueSongIds = [...new Set([...newPlaylist.songs.map(String), ...playlistSongs.map(String)])];
        newPlaylist.songs = uniqueSongIds;
        await newPlaylist.save();

        const populatedPlaylist = await Playlist.findById(newPlaylist._id).populate('songs');

        res.status(201).send({ message: 'Playlist created successfully with music.', playlist: populatedPlaylist });

    } catch (error) {
        console.error("Create playlist error:", error);
        if (error instanceof multer.MulterError) {
            return res.status(400).send({ message: 'File upload error during playlist creation.', error: error.message, field: error.field });
        } else {
...
app.get('/api/playlists', authenticateJWT, async (req, res) => {
    try {
        ...
        res.status(500).send({ message: 'Error fetching playlists.', error: error.message });
    }
});
...
app.get('/api/playlists/:playlistId', authenticateJWT, async (req, res) => {
    try {
        ...
        res.status(500).send({ message: 'Error fetching playlist details.', error: error.message });
    }
});

// Update Playlist Details (Protected, Owner-Only)
app.put('/api/playlists/:playlistId', authenticateJWT, authorizeOwner('playlist'), async (req, res) => {
    try {
        const playlistId = req.params.playlistId;
        ...
        res.status(200).send({ message: 'Playlist updated successfully.', playlist: updatedPlaylist });
    } catch (error) {
        console.error("Update playlist error:", error);
        res.status(500).send({ message: 'Error updating playlist.', error: error.message });
    }
});

// Delete Playlist (Protected, Owner-Only)
app.delete('/api/playlists/:playlistId', authenticateJWT, authorizeOwner('playlist'), async (req, res) => {
    try {
        const playlistId = req.params.playlistId;
...
app.post('/api/playlists/:playlistId/songs', authenticateJWT, authorizeOwner('playlist'), async (req, res) => {
    try {
        const playlistId = req.params.playlistId;
        ...
        const validSongIds = await Music.find({ _id: { $in: songIds }, userId: req.user.userId }).distinct('_id');
        const invalidSongIds = songIds.filter(id => !validSongIds.map(String).includes(id));

        if (invalidSongIds.length > 0) {
            return res.status(400).send({ message: `Invalid song IDs provided: ${invalidSongIds.join(', ')}. Songs must exist and belong to the user.` });
        }

        const updatedSongs = [...new Set([...playlist.songs.map(String), ...validSongIds.map(String)])];
        ...
        res.status(200).send({ message: 'Songs added to playlist successfully.', playlist: populatedPlaylist });
    } catch (error) {
...
app.delete('/api/playlists/:playlistId/songs/:songId', authenticateJWT, authorizeOwner('playlist'), async (req, res) => {
    try {
        const playlistId = req.params.playlistId;
        ...
        res.status(200).send({ message: 'Song removed from playlist successfully.', playlist: populatedPlaylist });
    } catch (error) {
        console.error("Remove song from playlist error:", error);
        res.status(500).send({ message: 'Error removing song from playlist.', error: error.message });
    }
});
...
app.get('/api/stream/:musicId', authenticateJWT, async (req, res) => {
    try {
        ...
        res.setHeader('Content-Disposition', `inline; filename="${music.title}.mp3"`);

        res.sendFile(audioPath);
    } catch (error) {
        console.error("Streaming error:", error);
        res.status(500).send({ message: 'Error streaming music.', error: error.message });
    }
});
...
app.get('/api/history', authenticateJWT, async (req, res) => {
    try {
        ...
        res.status(500).send({ message: 'Error fetching playback history.', error: error.message });
    }
});
...
app.post('/api/history', authenticateJWT, async (req, res) => {
    try {
        ...
        if (!musicId) {
            return res.status(400).send({ message: 'musicId is required to record history.' });
        }
        ...
        res.status(500).send({ message: 'Error recording playback history.', error: error.message });
    }
});
...
app.delete('/api/history', authenticateJWT, async (req, res) => {
    try {
        ...
        res.status(500).send({ message: 'Error clearing playback history.', error: error.message });
    }
});
